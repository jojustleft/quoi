...
def get_top(
    ...
    res = (
        df.group_by(entry_c).agg(method(pl.col(value_c))).sort(value_c, descending=True)
    )
    ...
    if calculate_share:
        if res[value_c].dtype.is_numeric() and df[value_c].min() < 0:
            ...
            res = res.with_columns(
                (pl.col(value_c) / pl.col(value_c).sum() * 100).alias("share")
            )
...
def fill_cartesian_expansion(
    ...
    if time_c is not None:
        ...
        if isinstance(df.schema[time_c], pl.Date):
            df_comb = pl.DataFrame(
                pl.date_range(
                    start=min_t, end=max_t, interval=interval, eager=True
                ).alias(time_c)
            )
        else:
