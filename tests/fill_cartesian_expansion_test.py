...
def _get_mock_data():
    df = pl.DataFrame([
        {'time': datetime(2025, 1, 1), 'country': 'PT', 'weather': 'sunny', 'val': 2},
        {'time': datetime(2025, 1, 3), 'country': 'FR', 'weather': 'cloudy', 'val': 10},
        {'time': datetime(2025, 1, 8), 'country': 'ES', 'weather': 'rain', 'val': 5},
        {'time': datetime(2025, 1, 15), 'country': 'FR', 'weather': 'foggy', 'val': 15}
    ])
...
def test_total_rows():
    df = _get_mock_data()
    expected_rows = ((df['time'].max() - df['time'].min()).days + 1) * df['country'].n_unique() * df['weather'].n_unique()
    print((df['time'].max() - df['time'].min()).days, df['country'].n_unique(), df['weather'].n_unique())
...
def test_default_value():
    ...
    new_rows = res.join(df, 
                        on=['time', 'country', 'weather'],
                        how='anti')
